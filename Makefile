...
lint: ## Reorders imports and runs the golangci-lint checker
	goimports -d -e -w ./cmd ./pkg/health/instrumenter.go ./pkg/health/server.go ./pkg/hello/server.go ./pkg/tools
	golangci-lint run
...
gen:
	...
	mv pkg/proto/health.pb.go pkg/health/healthpb/
	mv pkg/proto/health.pb.gw.go pkg/health/healthpb/
