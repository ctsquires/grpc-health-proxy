...
lint: ## Reorders imports and runs the golangci-lint checker
	goimports -d -e -w ./cmd ./pkg/health ./pkg/hello/server.go ./pkg/tools
	golangci-lint run
...
gen:
	...
	mv pkg/proto/health.pb.go pkg/grpc_health_proxy/
	mv pkg/proto/health.pb.gw.go pkg/grpc_health_proxy/
